; ThoCoin installer (Inno Setup). Build everything first with build-release.bat:
...
;   cargo build --release --bin thocoin-pool
;   copy target\release\thocoin-pool.exe thocoin-pool.exe
...
; Pool server (optional self-hosting). skipifsourcedoesntexist so the build still
; works if you haven't built thocoin-pool yet.
Source: "thocoin-pool.exe"; DestDir: "{app}"; Flags: ignoreversion skipifsourcedoesntexist
