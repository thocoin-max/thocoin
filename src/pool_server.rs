    ...
    for line in reader.lines() {
        ...
        match msg {
            ...
            ClientMsg::Share { nonce, timestamp, extranonce: _ } => {
                ...
                if addr.is_empty() {
                    send(&mut writer, &ServerMsg::Rejected { reason: "chưa subscribe".into() })?;
                    continue;
                }
                ...
                if !valid {
                    send(&mut writer, &ServerMsg::Rejected { reason: "share không đạt".into() })?;
                } else {
                    let (n, _) = pool.stats();
                    ...
                    if is_block {
                        match pool.on_block_won(&tmpl, nonce, timestamp) {
                            Ok(reward) => {
                                ...
                                println!("[★] BLOCK! height={h} reward={reward} bởi {addr}");
                                send(&mut writer, &ServerMsg::BlockFound { height: h, reward })?;
                            }
                            Err(e) => eprintln!("apply block lỗi: {e}"),
                        }
