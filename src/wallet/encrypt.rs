...
pub fn decrypt(blob: &[u8], passphrase: &str) -> Result<Vec<u8>> {
    ...
    if blob.len() < header + 16 {
        return Err(anyhow!("wallet file corrupt or too short"));
    }
    if &blob[..4] != MAGIC {
        return Err(anyhow!("not an encrypted wallet file"));
    }
    if blob[4] != VERSION {
        return Err(anyhow!("unsupported wallet version"));
    }
    ...
    let pt = cipher.decrypt(nonce, ct)
        .map_err(|_| anyhow!("wrong passphrase or data tampered"))?;
    key_bytes.zeroize();
    Ok(pt)
}
