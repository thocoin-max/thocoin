...
fn main() -> anyhow::Result<()> {
    ...
    if args.len() < 3 {
        eprintln!("Usage: thocoin-pool-miner <host:port> <THO_address> [threads]");
        std::process::exit(1);
    }
    ...
    println!("Connecting to pool {host} ...");
    ...
    println!("Subscribed with address {address}, {threads} threads");
        ...
        match msg {
            ServerMsg::Job { prev, merkle, bits, timestamp, share_bits, extranonce, height, .. } => {
                ...
                *job.write() = Some(JobData { prev: p, merkle: m, bits, timestamp, share_bits, extranonce });
                println!("New job: height {height}");
            }
            ServerMsg::Accepted { shares } => {
                println!("Share accepted (shares in window: {shares})");
            }
            ...
            ServerMsg::BlockFound { height, reward } => {
                ...
                println!("POOL WON BLOCK #{height}! reward {} (count {n}) - paid out by shares", reward);
            }
            ServerMsg::Error { message } => println!("Pool error: {message}"),
        }
    }
