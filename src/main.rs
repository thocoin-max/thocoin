...
async fn main() -> anyhow::Result<()> {
    ...
    let rpc = start_rpc(chain.clone(), wallet.clone(), mempool.clone());
    // The jsonrpc-http-server spins up its own tokio runtime internally. Dropping
    // it while we are inside this #[tokio::main] async context panics with
    // "Cannot drop a runtime in a context where blocking is not allowed".
    // Leak it so it is never dropped here; the OS reclaims it on process exit.
    std::mem::forget(rpc);
