...
#[derive(Debug, PartialEq, Clone, Copy)]
enum Tab { Overview, Send, Receive, Transactions, Addresses, Coins, Contacts, Console, Mining, PoolJoin, PoolCreate }
...
pub struct App {
    ...
    embedded_pool: Arc<crate::pool::embedded::EmbeddedPool>,
    join_pool_host: String,
    join_reward_addr: String,
    join_use_wallet: bool,
    join_show_advanced: bool,
    create_pool_name: String,
    create_pool_fee: String,
    create_pool_wallet: String,
    create_pool_minpay: String,
    create_pool_port: String,
}
...
impl App {
    pub fn new(
        ...
        let mempool_for_pool = mempool.clone();
        ...
        App {
            ...
            join_pool_host: OFFICIAL_POOL.into(),
            join_reward_addr: String::new(),
            join_use_wallet: true,
            join_show_advanced: false,
            create_pool_name: String::new(),
            create_pool_fee: String::new(),
            create_pool_wallet: String::new(),
            create_pool_minpay: String::new(),
            create_pool_port: String::new(),
        }
    }
                ...
                ui.allocate_ui_with_layout(avail,
                    ...
                    |ui| {
                        egui::ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
                            ...
                            ui.add_space(6.0);
                            ui.label(RichText::new("Write these 24 words down on paper and keep them somewhere safe.")
                            ...
                            ui.allocate_ui(Vec2::new(560.0, 0.0), |ui| {
                                ui.checkbox(&mut self.seed_ack_checkbox,
                                    RichText::new("I have written down my 24 words and stored them safely")
...
fn danger_btn(ui: &mut egui::Ui, t: &Theme, label: &str) -> egui::Response {
    ui.add(egui::Button::new(RichText::new(label).size(13.0).color(Color32::WHITE).strong())
        .fill(t.danger).stroke(Stroke::NONE)
        .rounding(Rounding::same(6.0)).min_size(Vec2::new(0.0, 36.0)))
}
fn pool_stat(ui: &mut egui::Ui, t: &Theme, label: &str, value: &str, width: f32) {
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ...
            ui.set_height(54.0);
            ui.vertical(|ui| {
                ui.label(RichText::new(label).size(11.0).color(t.text_dim));
                ui.add_space(6.0);
                ui.label(RichText::new(value).size(18.0).strong().color(t.text_strong));
            });
        });
}

/// Render three equal-width, equal-height stat cards on one row.
fn pool_stat_row(ui: &mut egui::Ui, t: &Theme, cells: [(&str, &str); 3]) {
    let gap = 10.0;
    let total = ui.available_width();
    // Subtract the two gaps and each card's frame border padding, then split.
    let w = ((total - gap * 2.0) / 3.0 - 30.0).max(40.0);
    ui.horizontal(|ui| {
        for (i, (label, value)) in cells.iter().enumerate() {
            pool_stat(ui, t, label, value, w);
                    ...
                    egui::Frame::none()
                        ...
                        .show(ui, |ui| {
                            match self.tab {
                                ...
                                Tab::PoolJoin     => self.pool_join_ui(ui),
                                Tab::PoolCreate   => self.pool_create_ui(ui),
                            }
                        });
                });
            });
...
impl App {
    ...
    fn sidebar_ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.add_space(10.0);
        ui.label(RichText::new("POOL MINING").size(10.0).color(t.text_dim).family(FontFamily::Proportional));
        ui.add_space(4.0);
        let pool_items: &[(Tab, &str, &str)] = &[
            (Tab::PoolJoin, "Join Pool", "⇄"),
            (Tab::PoolCreate, "Create Pool", "✚"),
        ];
        for (tab, label, icon) in pool_items.iter().copied() {
            sidebar_item(ui, &t, self.tab == tab, icon, label, || self.tab = tab);
            ui.add_space(2.0);
        }
    }
        ...
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ...
                ui.add(egui::TextEdit::multiline(&mut self.mnemonic_input)
                    ...
                    .hint_text("24 words separated by spaces")
                    .font(egui::TextStyle::Monospace).margin(Vec2::new(10.0, 8.0)));
                ui.add_space(12.0);
    ...
    fn pool_join_ui(&mut self, ui: &mut egui::Ui) {
        let t = self.theme;
        ui.label(RichText::new("Join Pool").size(20.0).strong().color(t.text_strong));
        ui.add_space(6.0);
        ui.label(RichText::new("Mine together with others and earn rewards proportional to your shares (PPLNS).")
            .size(12.0).color(t.text_dim));
        ui.add_space(16.0);

        let running = self.pool_client.is_running();
        let s = self.pool_client.stats.clone();
        ...
        egui::Frame::none()
            ...
            .rounding(Rounding::same(8.0)).inner_margin(Margin::same(16.0))
            .show(ui, |ui| {
                let on_official = self.join_pool_host.trim() == OFFICIAL_POOL;

                // Headline showing which pool will be joined.
                ui.horizontal(|ui| {
                    ui.label(RichText::new("⛏").size(16.0).color(t.accent));
                    ui.add_space(2.0);
                    if on_official {
                        ui.label(RichText::new("Official ThoCoin Pool").size(14.0).strong().color(t.text_strong));
                    } else {
                        ui.label(RichText::new("Custom Pool").size(14.0).strong().color(t.text_strong));
                    }
                });
                ui.add_space(4.0);
                ui.label(RichText::new(self.join_pool_host.trim()).size(12.0)
                    .color(t.text_dim).family(FontFamily::Monospace));
                ui.add_space(14.0);

                // Reward destination.
                ui.checkbox(&mut self.join_use_wallet, "Send rewards to my wallet");
                ui.add_space(6.0);
                ui.label(RichText::new("Reward address").size(11.5).color(t.text_dim));
                ui.add_space(4.0);
                if self.join_use_wallet {
                    let mut shown = self.wallet.address();
                    ui.add(egui::TextEdit::singleline(&mut shown)
                        .desired_width(ui.available_width())
                        .font(egui::TextStyle::Monospace)
                        .margin(Vec2::new(10.0, 8.0))
                        .interactive(false));
                } else {
                    ui.add_enabled(!running, egui::TextEdit::singleline(&mut self.join_reward_addr)
                        .desired_width(ui.available_width())
                        .hint_text("Enter a ThoCoin reward address")
                        .margin(Vec2::new(10.0, 8.0)));
                }
                ui.add_space(14.0);
                ...
                if !running {
                    let label = if on_official { "Join Official Pool" } else { "Start Pool Mining" };
                    if primary_btn(ui, &t, label).clicked() {
                        let host = self.join_pool_host.trim().to_string();
                        let addr = if self.join_use_wallet {
                            self.wallet.address()
                        } else {
                            self.join_reward_addr.trim().to_string()
                        };
                        if host.is_empty() {
                            self.notify("Enter a pool address", true);
                        } else if addr.is_empty() {
                            self.notify("Enter a reward address", true);
                        } else {
                            self.pool_client.start(host, addr);
                            self.notify("Connecting to pool...", false);
                        }
                    }
                } else {
                    if danger_btn(ui, &t, "Stop Pool Mining").clicked() {
                        self.pool_client.stop();
                        self.notify("Stopped pool mining", false);
                    }
                }

                // Advanced: choose a different pool.
                if !running {
                    ui.add_space(10.0);
                    let arrow = if self.join_show_advanced { "▾" } else { "▸" };
                    if ui.add(egui::Label::new(
                        RichText::new(format!("{arrow} Use a different pool")).size(11.5).color(t.text_dim))
                        .sense(egui::Sense::click())).clicked() {
                        self.join_show_advanced = !self.join_show_advanced;
                    }
                    if self.join_show_advanced {
                        ui.add_space(8.0);
                        ui.label(RichText::new("Pool address").size(11.5).color(t.text_dim));
                        ui.add_space(4.0);
                        ui.add(egui::TextEdit::singleline(&mut self.join_pool_host)
                            .desired_width(ui.available_width())
                            .hint_text("host:port")
                            .margin(Vec2::new(10.0, 8.0)));
                        ui.add_space(6.0);
                        if ghost_btn(ui, &t, "Reset to Official Pool").clicked() {
                            self.join_pool_host = OFFICIAL_POOL.into();
                        }
                    }
                }
            });
        ...
        // Live stats (only meaningful once started).
        if running {
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        let (dot, lbl) = if connected { (t.success, "Connected") } else { (t.warn, "Connecting...") };
                        ui.label(RichText::new("●").color(dot).size(10.0));
                        ui.label(RichText::new(lbl).size(12.5).strong().color(dot));
                    });
                    ui.add_space(10.0);
                    kv(ui, &t, "Worker ID", &worker);
                    kv(ui, &t, "Job height", &format!("{}", height));
                    kv(ui, &t, "Reward address", &short_addr(&if self.join_use_wallet { self.wallet.address() } else { self.join_reward_addr.clone() }));
                    kv(ui, &t, "Status", &status);
                });

            ui.add_space(8.0);
            ui.label(RichText::new("Rewards are paid by the pool to your reward address when the pool finds a block and your share of the PPLNS window matures.")
                .size(11.0).color(t.text_muted));
    ...
    fn pool_create_ui(&mut self, ui: &mut egui::Ui) {
        let t = self.theme;
        ui.label(RichText::new("Create Pool").size(20.0).strong().color(t.text_strong));
        ui.add_space(6.0);
        ui.label(RichText::new("Run your own mining pool. It uses this wallet's node and chain, so no extra software is needed.")
            .size(12.0).color(t.text_dim));
        ui.add_space(16.0);

        let running = self.embedded_pool.is_running();
        let s = self.embedded_pool.stats.clone();

        egui::Frame::none()
            .fill(t.panel_alt).stroke(Stroke::new(1.0, t.border))
            .rounding(Rounding::same(8.0)).inner_margin(Margin::same(16.0))
            .show(ui, |ui| {
                let field = |ui: &mut egui::Ui, label: &str, val: &mut String, hint: &str, enabled: bool| {
                    ui.label(RichText::new(label).size(11.5).color(t.text_dim));
                    ui.add_space(4.0);
                    ui.add_enabled(enabled, egui::TextEdit::singleline(val)
                        .desired_width(ui.available_width())
                        .hint_text(hint).margin(Vec2::new(10.0, 8.0)));
                    ui.add_space(12.0);
                };
                field(ui, "Pool Name", &mut self.create_pool_name, "Enter pool name", !running);
                field(ui, "Pool Fee (%)", &mut self.create_pool_fee, "Example: 1.0", !running);
                field(ui, "Pool Wallet", &mut self.create_pool_wallet, "Enter pool reward wallet (blank = this wallet)", !running);
                field(ui, "Minimum Payout (THO)", &mut self.create_pool_minpay, "Example: 10", !running);
                field(ui, "Stratum Port", &mut self.create_pool_port, "Example: 3333", !running);

                ui.add_space(4.0);
                if !running {
                    if primary_btn(ui, &t, "Start Pool").clicked() {
                        self.start_embedded_pool();
                    }
                } else {
                    if danger_btn(ui, &t, "Stop Pool").clicked() {
                        self.embedded_pool.stop();
                        self.notify("Pool stopped", false);
                    }
                }
            });
        ...
        if running {
            let url = s.pool_url.read().clone();
            let name = s.name.read().clone();
            let miners = s.online_miners.load(Ordering::Relaxed);
            let blocks = s.blocks_found.load(Ordering::Relaxed);
            let hr = s.pool_hashrate.load(Ordering::Relaxed) as f64;
            let status = s.last_status.read().clone();

            let phr_s = fmt_hashrate(hr);
            let miners_s = format!("{}", miners);
            let blocks_s = format!("{}", blocks);
            pool_stat_row(ui, &t, [
                ("Pool Hashrate", &phr_s),
                ("Online Miners", &miners_s),
                ("Blocks Found", &blocks_s),
            ]);
            ui.add_space(10.0);
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(RichText::new("●").color(t.success).size(10.0));
                        ui.label(RichText::new("Running").size(12.5).strong().color(t.success));
                    });
                    ui.add_space(10.0);
                    kv(ui, &t, "Pool name", &name);
                    kv(ui, &t, "Pool URL (local)", &url);
                    kv(ui, &t, "Miners connect to", &format!("<your-ip-or-domain>:{}", self.create_pool_port.trim()));
                    kv(ui, &t, "Status", &status);
                });

            ui.add_space(8.0);
            ui.label(RichText::new("Share this pool's address (your public IP or domain + port) so others can join. Make sure the port is open on your firewall/router.")
                .size(11.0).color(t.text_muted));
    ...
    fn start_embedded_pool(&mut self) {
        use crate::pool::embedded::PoolConfig;
        let port: u16 = match self.create_pool_port.trim().parse() {
            Ok(p) if p > 0 => p,
            _ => { self.notify("Enter a valid port (e.g. 3333)", true); return; }
        };
        let fee: f64 = self.create_pool_fee.trim().parse().unwrap_or(0.0);
        let minpay_tho: f64 = self.create_pool_minpay.trim().parse().unwrap_or(0.0);
        let min_payout = (minpay_tho * crate::core::consensus::COIN as f64) as u64;
        let name = if self.create_pool_name.trim().is_empty() {
            "ThoCoin Pool".to_string()
        } else { self.create_pool_name.trim().to_string() };

        let cfg = PoolConfig { name, fee_percent: fee, min_payout, port };
        match self.embedded_pool.start(cfg) {
            Ok(_) => self.notify(&format!("Pool started on port {}", port), false),
