...
impl Mempool {
    ...
    pub fn accept(&self, chain: &ChainState, tx: Transaction) -> Result<()> {
        if tx.is_coinbase() {
            return Err(anyhow!("coinbase not accepted into mempool"));
        }
        if tx.inputs.is_empty() || tx.outputs.is_empty() {
            return Err(anyhow!("tx missing input/output"));
        }
        if tx.has_duplicate_inputs() {
            return Err(anyhow!("tx has duplicate inputs"));
        }
        let size = tx.size();
        if size > MAX_BLOCK_SIZE {
            return Err(anyhow!("tx too large"));
        }

        {
            ...
            for input in &tx.inputs {
                if spent.contains(&input.prev) {
                    return Err(anyhow!("input already spent by another mempool tx"));
                }
            }
        }
        ...
        if tx_out > tx_in {
            return Err(anyhow!("tx outputs exceed inputs"));
        }
        drop(utxo);
        ...
        if fee < min_fee {
            return Err(anyhow!("fee below minimum ({} < {})", fee, min_fee));
        }
        ...
        if map.len() >= MAX_MEMPOOL {
            ...
            match lowest {
                ...
                _ => return Err(anyhow!("mempool full, fee too low")),
            }
        }
