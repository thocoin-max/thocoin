...
/// P2P listen port. Defaults to the network port, but can be overridden so a
/// second node (e.g. the mining pool) can run on the same machine as the wallet
/// without clashing on the bind port. The pool then dials the wallet via
/// THOCOIN_PEERS=127.0.0.1:22221.
fn p2p_listen_port() -> u16 {
    std::env::var("THOCOIN_P2P_PORT")
        .ok()
...
impl P2P {
    ...
    pub async fn run(self: Arc<Self>) -> anyhow::Result<()> {
        ...
        let listener = TcpListener::bind(("0.0.0.0", p2p_listen_port())).await?;
        loop {
            ...
            if !handshaked {
                match msg {
                    Msg::Version { magic, height } if magic == NETWORK_MAGIC => {
                        handshaked = true;
                        if height > self.best_height.load(Ordering::SeqCst) {
